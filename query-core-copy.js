(function () {
  ...
  // Populate queryStart/queryEnd using the shared populateTimes() from app.js
  function initTimes() {
    const qs = $("queryStart");
    const qe = $("queryEnd");
  ...
  function runQuery() {
    ...
    if (typeof queryAllSavedSchedulesDayRange !== "function") {
      render({ ok: false, reason: "Missing core query function (queryAllSavedSchedulesDayRange) in app.js" });
      return;
    }

    const res = queryAllSavedSchedulesDayRange(day, startStr, endStr);
    render(res);
  }
